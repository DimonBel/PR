 ...
 * if !isEmpty:
 * card carrying symbol, which faces up/down if isUp is true/false, which
 ...
 * Thread safety:
 * setOwner is always followed by Player.turned.add in synchronized block (TODO:
 * lock is on player. should be on card)
 * isEmpty is set to true only through remove() in synchronized block(TODO),
 * always following Player.relinguishAll() which removes owner
...
public class Card {
    ...
    /**
     * Replace the symbol on this card.
     *
     * @param newSymbol replacement symbol, must be non-null
     */
    public synchronized void replaceSymbol(String newSymbol) {
        if (newSymbol == null) {
            throw new IllegalArgumentException("card symbol cannot be null");
        }
        if (isEmpty) {
            return;
        }
    ...
    public String viewBy(Player player) {
        ...
        if (!isUp)
            return "down";
        else if (isControlled() && player.equals(getOwner()))
            return "my " + symbol;
        else
            return "up " + symbol;
    }
