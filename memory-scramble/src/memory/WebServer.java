 ...
 * 
 * <p>
 * PS4 instructions: the specifications of {@link #WebServer(Board, int)},
...
public class WebServer {
    ...
    public WebServer(Board board, int port) throws IOException {
        ...
        // HttpContext hello = server.createContext("/hello/", exchange ->
        // handleHello(exchange));
        ...
        // handle requests for /replace/player/card_old/card_new

        HttpContext replace = server.createContext("/replace/", exchange -> handleMap(board, exchange));
        replace.getFilters().addAll(filters);
    ...
    /*
     * Handle a request for /look/<playerName> by responding with string
     * representation of
    ...
    /*
     * Handle a request for /flip/<playerName>/<row>,<col> by modifying the game
     * state and
    ...
    /*
     * Handle a request for /watch/<playerName> by responding with string
     * representation of
    ...
    private void handleMap(Board board, HttpExchange exchange) throws IOException {
        ...
        final String remainder = path.substring(base.length());
        final String[] parts = remainder.split("/", 3);

        if (parts.length != 3 || parts[0].isEmpty()) {
            exchange.sendResponseHeaders(400, 0);
            try (PrintWriter out = new PrintWriter(new OutputStreamWriter(exchange.getResponseBody(), UTF_8), true)) {
                out.println("invalid replace request");
            }
            exchange.close();
            return;
        }

        final String playerName = urlDecode(parts[0]);
        final String oldSymbol = urlDecode(parts[1]);
        final String newSymbol = urlDecode(parts[2]);

        try {
            final String response = board.replaceSymbols(playerName, oldSymbol, newSymbol);
            exchange.sendResponseHeaders(200, 0);
            try (PrintWriter out = new PrintWriter(new OutputStreamWriter(exchange.getResponseBody(), UTF_8), true)) {
                out.println(response);
            }
